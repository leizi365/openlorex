...
export function FontColorToolbarButton({
  ...
  const onToggle = React.useCallback(
    (value = !open) => {
      ...
      if (!value) {
        ...
        if (savedSelectionRef.current || editor.selection) {
          setTimeout(() => {
            restoreSelection();
            editor.tf.focus();
          }, 0);
  ...
  const updateColor = React.useCallback(
    ...
    [editor, nodeType, restoreSelection]
  );
  ...
  const clearColor = React.useCallback(() => {
    ...
    onToggle(false);
  }, [editor, onToggle, nodeType, restoreSelection]);
  ...
  return (
    <DropdownMenu modal={false} onOpenChange={onToggle} open={open}>
      <DropdownMenuTrigger
        render={
          <ToolbarButton pressed={open} tooltip={tooltip} isDropdown size="sm" />
        }
      >
        {children}
      </DropdownMenuTrigger>
